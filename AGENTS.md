# Working agreement — mcp-replication-package
...
**Always paste the full PR URL** (`https://github.com/pathscale/mcp-replication-package/pull/<n>`), not just the number, so it's
clickable.
...
uncomment the rule below.
    ...
    ```bash
    gh pr view <number> --repo pathscale/mcp-replication-package --json statusCheckRollup
    ```
